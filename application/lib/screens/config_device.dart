...
class ConfigureDevice extends StatefulWidget {
  ...
  const ConfigureDevice({super.key, required this.deviceInformation});

  @override
  State<ConfigureDevice> createState() => _ConfigureDevice();
}

class _ConfigureDevice extends State<ConfigureDevice> {
  ...
  void initState() {
    super.initState();
    setState(() {
      _wifiSsid = widget.deviceInformation['network_ssid'];
      _samplingTime = widget.deviceInformation['sampling_time'].toString();
      _batteryMin = widget.deviceInformation['battery_min'].toString();
      _phMin = widget.deviceInformation['properties']['ph_min'].toString();
      _phMax = widget.deviceInformation['properties']['ph_max'].toString();
      _tempMin = widget.deviceInformation['properties']['te_min'].toString();
      _tempMax = widget.deviceInformation['properties']['te_max'].toString();
      _tdsMin = widget.deviceInformation['properties']['tds_min'].toString();
      _tdsMax = widget.deviceInformation['properties']['tds_max'].toString();
      _ecMin = widget.deviceInformation['properties']['ec_min'].toString();
      _ecMax = widget.deviceInformation['properties']['ec_max'].toString();
      _tuMin = widget.deviceInformation['properties']['tu_min'].toString();
      _tuMax = widget.deviceInformation['properties']['tu_max'].toString();
    });
    _isBluetoothEnabled();
  }
    ...
    switch (page) {
      ...
      case 1:
        return TurnOnBluetooth(
          ...
          title: "Configura ${widget.deviceInformation['device_name']}",
        );
      case 2:
      ...
      case 4:
        return DataValidation(
          ...
          bluetoothDevice: BluetoothDevice(
              address: widget.deviceInformation['bluetooth_macaddress']),
          deviceData: dataToMap(
              widget.deviceInformation['device_uuid'],
              widget.deviceInformation['device_name'],
              _wifiSsid,
              _wifiPassword,
              _serverUrl,
              _samplingTime,
